      ...
      <div className="readadmins_gridcont">
        {
          backAdminsData.map((item) => <div key={item?._id} className="admins_card">
            <div className="admin_imgcont">{item?.firstname.slice(0, 1).toLocaleUpperCase()}</div>
            <p className='admins_name'>{item?.firstname} {item?.lastname}</p>
            <ul className='admins_collection'>
              <li className="admins_item">Yoshi: <span>{item?.age} yosh</span></li>
              <li className="admins_item">Jinsi: <span>{item?.gender === "male" ? 'Erkak' : "Ayol"}</span></li>
              <li className="admins_item">Telefon: <span>+{item.phoneNumber}</span></li>
              <li className="admins_item">Username: <span>{item?.username}</span></li>
              <li className="admins_item">Password: <span>{item?.password}</span></li>
            </ul>
            <FaTrashAlt onClick={() => deleteAdmin(item?._id)} className='admins_delete' />
